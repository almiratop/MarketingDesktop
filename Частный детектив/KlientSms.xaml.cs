...
namespace Частный_детектив
{
    ...
    public partial class KlientSms : Window
    {
        ...
        int ClientId;
        TextBox body;
        public KlientSms(int zakazid, int clientid)
        {
            InitializeComponent();
            ZakazId = zakazid;
            ClientId = clientid;
        ...
        {
            Klient form = new Klient(ClientId);
            form.Show();
            this.Hide();
        }
            ...
            try
            {
                if (textBoxMessage.Text != "")
                {
                    if (textBoxMessage.Text.Length < 150)
                    {
                        ...
                        {
                            conn.Open();
                            MySqlCommand cmd1 = new MySqlCommand("select client_id from zakaz where zakaz_id = @zakaz_id LIMIT 1", conn);
                            cmd1.Parameters.AddWithValue("@zakaz_id", ZakazId);
                            int clientid = Convert.ToInt32(cmd1.ExecuteScalar().ToString());
                            ...
                            body = new TextBox();
                            body.Style = (Style)Resources["Text"];
                            body.Text = nameclient + "    |    " + textBoxMessage.Text;
                            stack.Children.Add(body);
                            scroll.ScrollToEnd();
                            textBoxMessage.Text = "";
                        }
        ...
        {
            var form = new KlientChats(ClientId);
            form.Show();
            this.Hide();
        }
    }
